...
public abstract class RTNode implements INode, Serializable
{
	 ...
	 * 成员变量1
	 * 结点所在的树
	 * transient修饰的成员变量不允许被序列化。我们要将其序列化所以就去掉
	 */
...
//		}
	 ...
	 * <b>分裂结点的平方算法</b>
	 * <p>
	 * 1、为两个组选择第一个条目(矩形)--调用算法pickSeeds()来为两个组选择第一个元素，分别把选中的两个条目(矩形)分配到两个组当中。<br>
	...
	/**
	 * 返回形成了最大冗余的矩形组合的uniqueID
	 *
	 * 1、对每一对条目(两个矩形)E1和E2，计算包围它们的Rectangle J(一个较大的矩形)，计算 冗余面积:d = area(J) - area(E1) - area(E2);<br>
